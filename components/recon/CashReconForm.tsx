...
export function CashReconForm({ outletId, date, onSaved }: { outletId: string; date: string; onSaved: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await request(`/api/cash-recon?${p}`)
  ...
  const closing = autoOpening + computed.cashCollected + computed.paidBillsCash - computed.cashExpenses - (Number(cashDeposited) || 0)
  ...
  const save = async () => {
    ...
    setBusy(true)
    try {
      await request('/api/cash-recon', { method: 'POST', body: JSON.stringify({ date, outletId, notes, cashDeposited: Number(cashDeposited) || 0, ...(canVerify && verifiedAmount !== '' ? { verifiedAmount: Number(verifiedAmount) || 0 } : {}) }) })
