...
import { isOwner, listPermissions, setPermission, RESOURCES, Resource, MANAGE_USERS_RESOURCES, BooleanResource } from '@/lib/rbac'
import { prisma } from '@/lib/prisma'

const VALID_RESOURCES = [...Object.values(RESOURCES), ...Object.values(MANAGE_USERS_RESOURCES)] as string[]
...
export async function GET(req: NextRequest) {
  ...
  const grants = await listPermissions(resource as Resource | BooleanResource)
...
export async function PUT(req: NextRequest) {
  ...
  const grant = await setPermission(resource as Resource | BooleanResource, userId, { canAdd: !!canAdd, canEdit: !!canEdit, canDelete: !!canDelete, canSettle: !!canSettle, canUnsettle: !!canUnsettle })
