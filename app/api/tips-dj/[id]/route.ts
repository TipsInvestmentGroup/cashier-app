...
/** Approve or reject a Tips/DJ bill request. body: { action: 'approve' | 'reject' } */
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!['TIPS', 'DJ'].includes(existing.billType)) return NextResponse.json({ error: 'Not a Tips/DJ bill' }, { status: 400 })
  ...
  await prisma.auditLog.create({
    data: { userId: user.userId, action: approvalStatus, entity: 'SignedBill', entityId: id, details: `${approvalStatus} ${existing.billType} bill for ${existing.personName} by ${user.name}` },
  })
