...
/** POST — reject a pending unlock request. Body: { comment }. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await prisma.businessDayUnlockRequest.update({
    where: { id },
    data: { status: 'REJECTED', approverId: user.userId, approverComment: body.comment || null, resolvedAt: new Date() },
  })
  ...
  await createNotification({
    ...
    message: `Your unlock request for ${request.businessDay.date.toISOString().slice(0, 10)} was rejected by ${actor.userName}${body.comment ? `: ${body.comment}` : '.'}`,
