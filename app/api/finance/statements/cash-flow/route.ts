...
import { cashFlowStatement } from '@/lib/finance-statements'
...
export async function GET(req: NextRequest) {
  ...
  if (!companyId) return NextResponse.json({ openingBalance: 0, closingBalance: 0, inflows: [], outflows: [], netChange: 0 })
  const now = new Date()
  ...
  return NextResponse.json(await cashFlowStatement(companyId, periodStart, periodEnd))
}
