...
interface Category { id: string; code: string; label: string; isActive: boolean }
...
export default function ProductCategoriesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [categories, access] = await Promise.all([request('/api/product-categories'), request('/api/persons-access')])
      setItems(categories || [])
  ...
  const add = async () => {
    ...
    try {
      await request('/api/product-categories', { method: 'POST', body: JSON.stringify({ label: newName.trim() }) })
  ...
  const saveEdit = async (id: string) => {
    try { await request(`/api/product-categories/${id}`, { method: 'PUT', body: JSON.stringify({ label: editValue.trim() }) }); toast.success('Saved'); setEditing(null); load() }
    catch (err: unknown) { toast.error(err instanceof Error ? err.message : 'Could not save') }
  }
  const toggle = async (c: Category) => {
    try { await request(`/api/product-categories/${c.id}`, { method: 'PUT', body: JSON.stringify({ isActive: !c.isActive }) }); load() }
    catch { toast.error('Could not update') }
  }
  const remove = async (c: Category) => {
    if (!confirm(`Delete category "${c.label}"?`)) return
    try { await request(`/api/product-categories/${c.id}`, { method: 'DELETE' }); toast.success('Deleted'); load() }
    ...
    <AppShell>
      ...
      <div className="space-y-6 max-w-2xl">
        <div>
          ...
          <p className="text-gray-500 text-sm">Categories used to group products — also drives which Cancellation Reasons apply to which products</p>
        </div>
          ...
          {canManage && (
            <div className="flex gap-2 mb-4">
              <input value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') add() }}
                placeholder="New category (e.g. Spirits)…"
        ...
        <p className="text-xs text-gray-400">Used by the Category picker on Products, and to scope Cancellation Reasons.</p>
      </div>
    </AppShell>
  )
}
