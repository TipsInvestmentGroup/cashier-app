...
export default function CancellationReasonsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [reasons, access] = await Promise.all([request('/api/cancellation-reasons'), request('/api/persons-access')])
      setItems(reasons || [])
    ...
    <AppShell>
      ...
      <div className="space-y-6 max-w-2xl">
        <div>
          ...
          <p className="text-gray-500 text-sm">Reasons available when recording a cancelled punch on Daily Collections</p>
        </div>
