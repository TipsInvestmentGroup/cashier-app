...
export default function PosHomePage() {
  ...
  const openShift = async (name: string) => {
    ...
    if (res.ok) {
      ...
      router.push(`/pos/tables?shiftId=${shift.id}`)
    }
    setBusy(false)
  }

  const goToTables = () => {
    if (activeShift) router.push(`/pos/tables?shiftId=${activeShift.id}`)
  }
        ...
        {activeShift ? (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="font-bold text-green-800 text-lg">
              Shift {activeShift.name} — Iko Wazi
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {SHIFTS.map(s => (
                <button
                  ...
                  disabled={busy}
                  className={`${s.color} text-white rounded-2xl p-5 text-left hover:opacity-90 active:scale-95 transition-all disabled:opacity-50`}
                >
